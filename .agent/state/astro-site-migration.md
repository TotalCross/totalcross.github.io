...
- Next concrete action: commit and push the npm 11 lockfile portability correction to
  draft PR #12, then require the rerun validation/artifact job to pass before
  creating/promoting `main` or changing the Pages source.
- Validation completed: M0-M4 and M6 are closed. M5 local Node 24 validation passed
  ...
  PR #12's first run failed only at `npm ci` because the Mac-generated lock omitted
  two Linux optional runtime entries. Regeneration with Node 24/npm 11.16 added them;
  the exact clean Ubuntu-style install now succeeds in a detached worktree.
